 ...
 */
class TSA_Blog_Model_Wysiwyg_Config extends Varien_Object
{
    /**
    ...
    public function getConfig($data = array())
    {
        ...
        $config->setData(array(
            ...
            'directives_url'                => Mage::getSingleton('adminhtml/url')->getUrl('*/tsa_blog_wysiwyg/directive'),
            'popup_css'                     =>
        ...
        if (Mage::getSingleton('admin/session')->isAllowed('tsa_blog/media_gallery')) {
            $config->addData(array(
                'add_images'               => true,
                'files_browser_window_url' => Mage::getSingleton('adminhtml/url')->getUrl('*/tsa_blog_wysiwyg_images/index'),
                'files_browser_window_width'
                    => (int) Mage::getConfig()->getNode('adminhtml/tsa_blog/browser/window_width'),
                'files_browser_window_height'
                    => (int) Mage::getConfig()->getNode('adminhtml/tsa_blog/browser/window_height'),
            ));
        }
        ...
        Mage::dispatchEvent('tsa_blog_wysiwyg_config_prepare', array('config' => $config));
    ...
    public function isEnabled()
    {
        ...
        if (!is_null($storeId)) {
            $wysiwygState = Mage::getStoreConfig('tsa_blog/wysiwyg/enabled', $storeId);
        } else {
            $wysiwygState = Mage::getStoreConfig('tsa_blog/wysiwyg/enabled');
        }
    ...
    {
        return Mage::getStoreConfig('tsa_blog/wysiwyg/enabled') == self::WYSIWYG_HIDDEN;
    }
}
