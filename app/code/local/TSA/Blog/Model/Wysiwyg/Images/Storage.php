 ...
 */
class TSA_Blog_Model_Wysiwyg_Images_Storage extends Varien_Object
{
    ...
    {
        $collection = Mage::getModel('tsa_blog/wysiwyg_images_storage_collection');
        if ($path !== null) {
    ...
    public function createDirectory($name, $path)
    {
        if (!preg_match(self::DIRECTORY_NAME_REGEXP, $name)) {
            Mage::throwException(Mage::helper('tsa_blog')->__('Invalid folder name. Please, use alphanumeric characters, underscores and dashes.'));
        }
        ...
        if (file_exists($newPath)) {
            Mage::throwException(Mage::helper('tsa_blog')->__('A directory with the same name already exists. Please try another folder name.'));
        }
        ...
        Mage::throwException(Mage::helper('tsa_blog')->__('Cannot create new directory.'));
    }
    ...
    public function deleteDirectory($path)
    {
        ...
        if ($rootCmp == $pathCmp) {
            Mage::throwException(Mage::helper('tsa_blog')->__('Cannot delete root directory %s.', $path));
        }
        ...
        if (!$io->rmdir($path, true)) {
            Mage::throwException(Mage::helper('tsa_blog')->__('Cannot delete directory %s.', $path));
        }
    ...
    public function uploadFile($targetPath, $type = null)
    {
        ...
        if (!$result) {
            Mage::throwException( Mage::helper('tsa_blog')->__('Cannot upload file.') );
        }
    ...
    {
        return Mage::helper('tsa_blog/wysiwyg_images');
    }
    ...
    {
        if (! $this->_config) {
            $this->_config = Mage::getConfig()->getNode('tsa_blog/browser', 'adminhtml');
        }
